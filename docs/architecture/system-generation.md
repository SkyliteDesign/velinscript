# System-Generierung (3.5.0)

Der SystemGenerator erstellt derzeit stabile REST/Axum-Grundgerüste. Weitere Systemkomponenten wie Auth, Datenbank und AI werden je nach Bereich als Experimental geführt.

---

## Übersicht

`velin generate system` erzeugt aus Velin-Quellen Komponenten-Dateien und ein Axum-`Cargo.toml`/`src/main.rs`-Scaffold — kein Anspruch auf ein „vollständiges Produktionssystem“ ohne nachweisbaren E2E-Pfad.

- **REST** (`@GET`/`@POST`/…): stabiler Kern — Build und HTTP-Request.
- **Auth / Database / AI**: zusätzliche Template-Dateien möglich; Experimental.

## CLI

```bash
velin generate system --path main.velin -o generated_system
cd generated_system && cargo run
```
