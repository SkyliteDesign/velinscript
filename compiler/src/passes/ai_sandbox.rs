/// AI Code Sandbox — validate-only (no real execution)
///
/// Checks AI-generated Velin for forbidden patterns and structure.
/// Does **not** execute code in an OS/container sandbox.
...
/// AI Code Sandbox (static validation)
pub struct AICodeSandbox {
    allowed_functions: HashSet<String>,
    forbidden_functions: HashSet<String>,
    max_execution_time: u64, // reserved; not used for wall-clock execution
}

/// Sandbox validation result (not a runtime execution result)
#[derive(Debug, Clone)]
...
impl AICodeSandbox {
    ...
    /// Static validation only (no process/container execution).
    pub fn validate_only(&self, code: &str) -> Result<SandboxResult> {
        self.execute_safely(code)
    }

    /// Alias for validate_only (name kept for callers; does not execute).
