...
impl IROptimizer {
    ...
    pub fn optimize(&self, module: &mut IRModule) {
        ...
        for _ in 0..3 {
            if self.enabled_optimizations.contains(&Optimization::DeadCodeElimination) {
                self.eliminate_dead_code(module);
            }
            
            if self.enabled_optimizations.contains(&Optimization::ConstantFolding) {
                self.fold_constants(module);
            }
            
            if self.enabled_optimizations.contains(&Optimization::FunctionInlining) {
                self.inline_functions(module);
            }
            
            if self.enabled_optimizations.contains(&Optimization::LoopOptimization) {
        ...
        for instruction in &block.instructions {
            match instruction {
                ...
                IRInstruction::Or { left, right, .. } => {
                    if !used.contains(&left) { used.push(left.clone()); }
                    if !used.contains(&right) { used.push(right.clone()); }
                }
                IRInstruction::Not { operand, .. } => {
                    if !used.contains(&operand) { used.push(operand.clone()); }
                }
                IRInstruction::Load { source, .. } => {
                    if !used.contains(&source) { used.push(source.clone()); }
                }
                IRInstruction::Store { dest, value } => {
                    if !used.contains(&dest) { used.push(dest.clone()); }
                    if !used.contains(&value) { used.push(value.clone()); }
                }
                IRInstruction::Branch { condition, .. } => {
                    if !used.contains(&condition) { used.push(condition.clone()); }
                }
                IRInstruction::Return { value } => {
                    if let Some(v) = value {
                        if !used.contains(v) { used.push(v.clone()); }
                    }
                }
                IRInstruction::Call { func, args, .. } |
                IRInstruction::CallAsync { func, args, .. } => {
                    if !used.contains(func) { used.push(func.clone()); }
                    for arg in args {
                        if !used.contains(arg) { used.push(arg.clone()); }
                    }
                }
                IRInstruction::StructAccess { struct_val, .. } => {
                    if !used.contains(&struct_val) { used.push(struct_val.clone()); }
                }
                IRInstruction::ListGet { list, index, .. } => {
                    if !used.contains(&list) { used.push(list.clone()); }
                    if !used.contains(&index) { used.push(index.clone()); }
                }
                IRInstruction::MapGet { map, key, .. } => {
                    if !used.contains(&map) { used.push(map.clone()); }
                    if !used.contains(&key) { used.push(key.clone()); }
                }
                IRInstruction::StructConstruct { fields, .. } => {
                    for (_, val) in fields {
                        if !used.contains(&val) { used.push(val.clone()); }
                    }
                }
                IRInstruction::EnumConstruct { data, .. } => {
                    if let Some(d) = data {
                        if !used.contains(d) { used.push(d.clone()); }
                    }
                }
                IRInstruction::Match { value, arms } => {
                    ...
                    for arm in arms {
                        if let Some(guard) = &arm.guard {
                            if !used.contains(guard) { used.push(guard.clone()); }
                        }
                    }
                }
        ...
        for instruction in &mut func.body.instructions {
            match instruction {
                IRInstruction::Add { dest, left, right } => {
                    if let (IRValue::Constant(IRConstant::Number(a)), IRValue::Constant(IRConstant::Number(b))) = (left, right) {
                ...
                IRInstruction::And { dest, left, right } => {
                    if let (IRValue::Constant(IRConstant::Boolean(a)), IRValue::Constant(IRConstant::Boolean(b))) = (left, right) {
                        *instruction = IRInstruction::Store {
                            dest: dest.clone(),
    ...
    fn inline_function_calls(&self, func: &mut IRFunction, small_functions: &HashMap<String, IRFunction>) {
        ...
        for instruction in &func.body.instructions {
            match instruction {
                IRInstruction::Call { dest, func: func_val, args } => {
                    ...
                    if let IRValue::Variable(var) = func_val {
                        if let Some(target_func) = small_functions.get(&var.name) {
                            ...
                            let inlined = self.inline_function(target_func, args, &mut temp_id_counter);
    ...
    fn inline_function(&self, target_func: &IRFunction, args: &[IRValue], temp_id_counter: &mut usize) -> Vec<IRInstruction> {
    ...
    fn unroll_small_loops(&self, block: &mut IRBlock) {
        ...
        while i < block.instructions.len() {
            ...
            if let IRInstruction::Branch { condition: _, then_block, else_block: _ } = instruction {
    ...
    fn eliminate_induction_variables(&self, block: &mut IRBlock) {
        ...
        for instruction in &block.instructions {
            ...
            if let IRInstruction::Add { dest, left, right } = instruction {
                if let (IRValue::Variable(var1), IRValue::Constant(IRConstant::Number(1.0))) = (left, right) {
