...
pub struct IRCodeGenerator {
    ...
    /// HTTP framework for Rust lowering (axum default, actix when set)
    framework: Option<String>,
}

impl IRCodeGenerator {
    /// Erstellt einen neuen IR Code Generator (default framework: axum)
    ...
    /// Generiert Rust-Code aus IR (HTTP/Auth via framework-specific lowering)
    fn generate_rust(&self, module: &IRModule) -> String {
        let mut output = String::new();
        let routes = lowering::collect_routes(module);
        let http = !routes.is_empty();
        let use_actix = lowering::is_actix_framework(self.framework());

        if http {
            if use_actix {
                output.push_str(lowering::actix_imports());
            } else {
                output.push_str(lowering::axum_imports());
            }
        } else {
        ...
        if http {
            if use_actix {
                output.push_str(&lowering::generate_actix_auth_middleware(&routes));
            } else {
                output.push_str(&lowering::generate_auth_middleware(&routes));
            }
        }
        ...
        if http {
            if use_actix {
                output.push_str(&lowering::generate_actix_router(&routes));
            } else {
                output.push_str(&lowering::generate_axum_router(&routes));
            }
        }
    ...
    /// Generiert eine Rust-Funktion (Attribute → Docs / Axum-Signatur bei HTTP)
    fn generate_rust_function(&self, func: &IRFunction) -> String {
        let mut output = String::new();

        output.push_str(&lowering::attribute_doc_comments(&func.attributes));
        ...
        let snake_name = lowering::to_snake_case(&func.name);
        let route = lowering::route_from_function(func);
        let async_kw = if func.is_async || route.is_some() {
            "async "
        } else {
            ""
        };

        output.push_str(&format!("{}{}fn {}(", vis, async_kw, snake_name));

        if let Some(ref route) = route {
            output.push_str(&lowering::axum_params(func, route));
        } else {
            let params: Vec<String> = func
                ...
                .map(|p| {
                    format!(
                        "{}: {}",
                        lowering::to_snake_case(&p.name),
                        self.ir_type_to_rust(&p.ty)
                    )
                })
                .collect();
    ...
    fn generate_php(&self, module: &IRModule) -> String {
        ...
        let routes = lowering::collect_routes(module);
        if !routes.is_empty() {
            output.push_str("use Illuminate\\Support\\Facades\\Route;\n\n");
            for r in &routes {
                let method = r.method.to_lowercase();
                output.push_str(&format!(
                    "Route::{}('{}', '{}');\n",
                    method, r.path, r.handler
                ));
            }
            output.push_str("\n");
        }
    ...
    fn generate_python(&self, module: &IRModule) -> String {
        ...
        let routes = lowering::collect_routes(module);
        if !routes.is_empty() {
            output.push_str("from fastapi import FastAPI\n\n");
            output.push_str("app = FastAPI()\n\n");
        } else {
            output.push_str("\n");
        }
        ...
        // Functions generieren
        for f in &module.functions {
            if let Some(route) = lowering::route_from_function(f) {
                output.push_str(&format!(
                    "@app.{}(\"{}\")\n",
    ...
    fn generate_javascript(&self, module: &IRModule) -> String {
        ...
        let routes = lowering::collect_routes(module);
        if !routes.is_empty() {
            output.push_str("const express = require('express');\n");
            output.push_str("const app = express();\n\n");
        } else {
            output.push_str("\n");
        }
    ...
    fn generate_typescript(&self, module: &IRModule) -> String {
        ...
        let routes = lowering::collect_routes(module);
        if !routes.is_empty() {
            output.push_str("import express from 'express';\n");
            output.push_str("const app = express();\n\n");
        } else {
            output.push_str("\n");
        }
    ...
    fn generate_go(&self, module: &IRModule) -> String {
        ...
        let routes = lowering::collect_routes(module);
        if !routes.is_empty() {
            output.push_str("import \"github.com/gin-gonic/gin\"\n\n");
        }
        ...
        if !routes.is_empty() {
            output.push_str("func setupRouter() *gin.Engine {\n");
            output.push_str("    r := gin.Default()\n");
            for r in &routes {
                ...
                output.push_str(&format!(
                    "    r.{}(\"{}\", func(c *gin.Context) {{ c.JSON(200, {}) }} )\n",
                    method, r.path, r.handler
                ));
            }
    ...
    fn generate_java(&self, module: &IRModule) -> String {
        ...
        let routes = lowering::collect_routes(module);
        if !routes.is_empty() {
            output.push_str("import org.springframework.web.bind.annotation.*;\n\n");
            output.push_str("@RestController\n");
        }
        ...
        for f in &module.functions {
            if let Some(route) = lowering::route_from_function(f) {
                let ann = match route.method.to_uppercase().as_str() {
                    "POST" => "PostMapping",
                    "PUT" => "PutMapping",
    ...
    fn generate_csharp(&self, module: &IRModule) -> String {
        ...
        let routes = lowering::collect_routes(module);
        if !routes.is_empty() {
            output.push_str("using Microsoft.AspNetCore.Mvc;\n\n");
            output.push_str("[ApiController]\n");
            output.push_str("[Route(\"api\")]\n");
        } else {
            output.push_str("\n");
        }
        output.push_str("public class MainController {\n\n");
