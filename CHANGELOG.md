...
- Der Compiler ist über den Befehl `velin` verfügbar (`velin --version`, `check`, `compile`, `init` / `new`).
- HTTP-APIs mit `@GET`, `@POST` und weiteren Methoden erzeugen Rust-Code mit Axum-Router und Handlern.
- `velin init` / `velin new` erzeugen ein lauffähiges Axum-Gerüst (`Cargo.toml`, `src/main.rs`, Hello-Endpoint).
- `@Auth` und `@Role` schützen Endpunkte im generierten Rust/Axum-Code (401 ohne Authorization-Header).
- Named-Argumente an Decorators (z. B. `@Cache(ttl = "60")`) werden korrekt verarbeitet.
- HTTP-Routen-Emission auch für PHP, Python, TypeScript, JavaScript, Go, Java und C# (Entwicklungs-/Experimentalstatus; stabiler Laufzeitpfad: Rust/Axum).
- Security-Gate: Warnungen/Fehler bei hartcodierten Secrets und unsicheren Pfaden.
- `velin generate system` erstellt stabile REST/Axum-Grundgerüste; weitere Systemkomponenten (Auth/DB/AI) je nach Bereich Experimental.
- `velin backup` sichert `.velin`-Projektdateien unter `.velin/backup`.
- Geprüft unter Windows und Linux für den stabilen Hello-/Auth-Pfad.

### Fixed

- `@Auth` schützt Routen gezielt; ungeschützte Routen werden nicht mehr versehentlich in die Auth-Middleware gezogen.
- Doppelte Routen/Funktionen nach automatischer Code-Ordnung (Toposort) behoben.
- Installations-Prefix respektiert das gewählte Zielverzeichnis für die Binaries.

### Changed

- Fokus von 3.5.0: stabiler API-Entwicklungsweg mit Rust/Axum. Weitere Target-Runtimes und erweiterte Auth liegen außerhalb dieses Stable-Umfangs (siehe Docs).
