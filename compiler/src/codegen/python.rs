...
impl CodeGenerator for PythonCodeGenerator {
    fn generate(&mut self, program: &Program, config: &CodegenConfig) -> anyhow::Result<String> {
        ...
        let app_init = FrameworkSelector::generate_python_app_init(self.framework, self.routes.clone());
...
impl PythonCodeGenerator {
    ...
    fn generate_function(&mut self, f: &Function) {
        ...
        let return_type = f.return_type.as_ref().map(|t| format!(" -> {}", self.map_type(t))).unwrap_or_else(|| " -> None".to_string());
        
        let params: Vec<String> = f.params.iter()
            .map(|p| format!("{}: {}", p.name, self.map_type(&p.param_type)))
            .collect();

        self.writeln(&format!("def {}({}){}:", f.name, params.join(", "), return_type));
        self.indent();
    ...
    fn generate_statement(&mut self, stmt: &Statement) {
        match stmt {
            ...
            Statement::If(if_stmt) => {
                ...
                if if_stmt.then_block.statements.is_empty() { self.writeln("pass"); }
                self.dedent();
                ...
                if let Some(else_block) = &if_stmt.else_block {
                    ...
                    if else_block.statements.is_empty() { self.writeln("pass"); }
                    self.dedent();
                }
            },
    ...
    fn generate_expression(&mut self, expr: &Expression) {
        match expr {
            ...
            Expression::If { condition, then_expr: _, else_expr } => {
                self.write("(");
    ...
    fn map_type(&self, ty: &Type) -> String {
        match ty {
            ...
            Type::Map { key, value } => format!("Dict[{}, {}]", self.map_type(key), self.map_type(value)),
            Type::Named(name) => {
                match name.as_str() {
