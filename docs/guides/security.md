...
1.  [Authentifizierung & Autorisierung](#1-authentifizierung--autorisierung)
    ...
    *   [`@Role` (markiert geschützt)](#role-markiert-geschützt)
...
Ohne `Authorization`-Header: `401 Unauthorized`. Mit Header (z. B. `Bearer …`): Request erreicht den Handler (Header-Präsenz, keine Token-Validierung im Stable-Pfad).
...
Optionaler Pass prüft KI-generierten Velin-Code **statisch** (erlaubte/verbotene Aufrufe, AST-Walk). Es handelt sich **nicht** um OS-/Container-Execution.

**Beispiel:**
```rust
let sandbox = AICodeSandbox::new();
match sandbox.validate_only(generated_code) {
