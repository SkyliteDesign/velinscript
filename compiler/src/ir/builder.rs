...
impl IRBuilder {
    ...
    /// Konvertiert Decorators zu Attributes (inkl. Named-Arg-Values)
    fn build_attributes(&self, decorators: &[Decorator]) -> Vec<IRAttribute> {
        decorators
            .iter()
            .map(|d| IRAttribute {
                name: d.name.clone(),
                args: d.args.iter().map(|a| self.decorator_arg_to_ir(a)).collect(),
            })
            .collect()
    }

    fn decorator_arg_to_ir(&self, arg: &DecoratorArg) -> IRAttributeArg {
        match arg {
            ...
            DecoratorArg::Named { name, value } => IRAttributeArg::Named {
                name: name.clone(),
                value: Box::new(self.decorator_arg_to_ir(value)),
            },
        }
    }
