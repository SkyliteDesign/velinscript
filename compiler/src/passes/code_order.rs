...
impl CodeOrderingPass {
    ...
    fn order_program(&self, program: &mut Program) -> Result<()> {
        ...
        // Keep unnamed items (use / top-level) and any named items that were not in the graph.
        // Do NOT re-append everything: after the loop above, item_map_mut is empty, so a
        // "remaining names" check would wrongly duplicate every function/struct.
        for item in &program.items {
            match self.get_item_name(item) {
                Some(name) => {
                ...
                None => ordered_items.push(item.clone()),
            }
        }
